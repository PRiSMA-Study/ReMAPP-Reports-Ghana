...
*Table 1.a: Enrollment numbers for PRiSMA MNH Study
	...
	****M01_PC_IEORRES and M01_EXCL_YN_IEORRES are string variables
		
	*Creating variable for enrolled for PRiSMA - this is assuming you cannot consent and not enroll
		gen enrolled_prisma=.
		replace enrolled_prisma=1 if (M01_CON_SIGNATURE_PRYN=="1" & M01_CON_YN_DSDECOD==1)
		replace enrolled_prisma=0 if (M01_CON_SIGNATURE_PRYN!="1" | M01_CON_YN_DSDECOD!=1)
		label var enrolled_prisma "Participant enrolled for PRiSMA"
		label values enrolled_prisma yesno
		
	******M01_CON_SIGNYN_DSDECOD is M01_CON_SIGNATURE_PRYN[str7] in GHANA SITE 
...
*Demographic variables for PRiSMA MNH Study 
	...
	*Creating maternal age at enrollment variable - using date at enrollment  
		...
		*cleaning date variables
		gen dob=date(M01_BRTHDAT , "YMD")
		gen scrn_date=date(M01_SCRN_OBSSTDAT, "YMD")
		format scrn_date %d
	****The date format for the GHANA site is YMD
	...
	****Date format for GHANA site is YMD
...
*Table 1.1: Aim 1 inclusion criteria for those enrolled in PRiSMA
	...
	****The variable M06_SINGLE_FAORRES1 is byte and not string
	...
	*7. Normal blood pressure - at enrollment
		foreach x of varlist  M17_BP_SYS_VSORRES1  M17_BP_DIA_VSORRES1 {
		replace `x' = "" if(`x' =="SKIPPED" | `x' == "UNDEFINED")
		destring `x', replace 
		}
		
	******M08_BPAVG_SYS_VSORRES1 and M08_BPAVG_DIA_VSORRES1 are not string variables
...
/*		 
	...
	*****M03b_PH2_BS_RPORRES and M03b_PH3_BS_RPORRES are not string variables
*/	
		 gen miscarriage_1=0
	...
	*9. No preterm delivery or low birthweight 
		foreach x of varlist M03b_LB_WEIGHT  M03b_LB2_WEIGHT M03b_LB3_WEIGHT M04_PRETERM_RPORRES* M03b_LB_LENGTH_RPORRES ///
		M03b_LB2_LENGTH_RPORRES M03b_LB3_LENGTH_RPORRES {
			replace `x' = "" if(`x' =="SKIPPED" | `x' == "UNDEFINED")
			destring `x', replace 
			}
		*****M04_LOWBIRTHWT_RPORRES1 is not a string variable
		...
		*Any lowbirthweight
		gen any_m04_lbw=.
		replace any_m04_lbw=0 if M04_LOWBIRTHWT_RPORRES1==0
		...
		****M04_LOWBIRTHWT_RPORRES2 to M04_LOWBIRTHWT_RPORRES6 is not in the data
		...
		***replace any_preterm=9 if (M03a_PH_PREV_RPORRES==1 & (M04_PRETERM_RPORRES1==99 & M04_PRETERM_RPORRES2==99 | M04_PRETERM_RPORRES3==99 & M04_PRETERM_RPORRES4==99 | M04_PRETERM_RPORRES5==99 | M04_PRETERM_RPORRES6==99))
		...
		replace any_preterm=9 if (M03a_PH_PREV_RPORRES==1 & (M04_PRETERM_RPORRES1==99 ))
